...
const countries = computed(() => {
  const uniqueCountries = [...new Set(users.value.map(u => u.address?.country).filter(Boolean))];
  return uniqueCountries.map(c => ({ label: c, value: c })).sort((a, b) => a.label.localeCompare(b.label));
});
  ...
  <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6">
      <IconField class="w-full md:w-64">
        ...
        <InputText v-model="filters['global'].value" placeholder="Buscar por nombre o email" fluid />
      </IconField>
    ...
    >
      <template #role="{ data }">
          <span 
            ...
            :class="data.role === 'Admin' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'"
          >
            {{ data.role }}
          </span>
      </template>
