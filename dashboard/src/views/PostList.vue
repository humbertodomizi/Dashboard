...
const loadPosts = async () => {
    ...
    try {
        ...
        posts.value = rawPosts.map(post => ({
            ...post,
            userName: post.user ? `${post.user.firstName} ${post.user.lastName}` : 'Desconocido',
            userEmail: post.user ? post.user.email : '',
            publishedDateObj: post.publishedAt ? new Date(post.publishedAt) : null
        }));
    } catch (error) {
        console.error(error);
        toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudieron cargar los posts', life: 3000 });
    } finally {
...
const categories = computed(() => {
    const uniqueCategories = [...new Set(posts.value.flatMap(p => p.categories || []))];
...
const filters = ref({
  ...
  publishedDateObj: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.DATE_IS }] },
});

const columns = [
  { field: 'title', header: 'Título', sortable: true },
  { field: 'userName', header: 'Autor', sortable: true, slotName: 'author' },
  { field: 'categories', header: 'Categorías', sortable: false, slotName: 'categories' },
...
const handlePublish = async (post) => {
    if (post.status === 'Published') {
        toast.add({ severity: 'info', summary: 'Info', detail: 'El post ya está publicado', life: 3000 });
        return;
    }
    
    try {
        await api.updatePost(post._id, { status: 'Published' });
        toast.add({ severity: 'success', summary: 'Éxito', detail: 'Post publicado correctamente', life: 3000 });
        await loadPosts();
    } catch (error) {
        console.error(error);
        toast.add({ severity: 'error', summary: 'Error', detail: 'Error al publicar el post', life: 3000 });
    }
};
...
const deletePost = async (post) => {
    try {
        ...
        toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo eliminar el post', life: 3000 });
    }
};
  ...
  <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full">
    ...
    <div class="flex flex-col md:flex-row gap-4 mb-6">
      <IconField class="w-full md:w-64">
        ...
        <InputText v-model="filters['global'].value" placeholder="Buscar post..." fluid />
      </IconField>
    ...
    >
        ...
        <template #categories="{ data }">
            <div class="flex gap-1 flex-wrap">
                <span v-for="cat in data.categories" :key="cat" class="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-xs">
                    {{ cat }}
                </span>
            </div>
        </template>
