...
const showBackButton = computed(() => {
  ...
  return route.name && (route.name.includes('-create') || route.name.includes('-edit'));
});
...
const getPageTitle = computed(() => {
  switch (route.name) {
    ...
    case 'user-create': return 'Nuevo Usuario';
    case 'user-edit': return 'Editar Usuario';
    case 'posts': return 'Posts';
    case 'post-create': return 'Nuevo Post';
    case 'post-edit': return 'Editar Post';
...
<template>
  <div class="flex h-screen bg-gray-50 font-sans text-gray-900">
    ...
    <main class="flex-1 flex flex-col overflow-hidden transition-all duration-300">
      <!-- Top Bar -->
      <header class="h-16 bg-white shadow-sm flex items-center px-6 justify-between border-b border-gray-100">
